...
class VentanaPrincipal(QMainWindow):
    ...
    def construirGrafoEscaladaSimple(self):
        ...
        if len(solucionEscaladaSimple) == 1:
            posEscaladaSimple = {node.nombre: (0, 0) for node in solucionEscaladaSimple}
            for nodo in solucionEscaladaSimple:
                grafoEscaladaSimple.add_node(nodo.nombre)
    ...
    def construirGrafoMaximaPendiente(self):
        ...
        if len(solucionMaximaPendiente) == 1:
            posMaximaPendiente = {nodo.nombre: (0, 0) for nodo in solucionMaximaPendiente}
            for nodo in solucionMaximaPendiente:
                grafoMaximaPendiente.add_node(nodo.nombre)
    ...
    def maximaPendiente(self, estadosBuscar):
        ...
        # Se define estado inicial como el estado actual, antes de empezar el recorrido.
        estadoActual = self.estadoInicial
        ...
        while not fin and solucionEncontrada != self.estadoFinal:
            ...
            # MÁXIMA PENDIENTE: Busca el mejor sucesor
            sucesor = estadoActual.valor
            relacionesOrdenadas = self.ordenarEstados(estadoActual.relaciones)
            for relacion in relacionesOrdenadas:
                if relacion == self.estadoFinal:
                    # Si la relación es el estado final, se añade y se termina
                    relacionAgregarSolucion = Estado(relacion.nombre, relacion.valor, relacion.posicion)
                    estadoAgregarSolucion.agregarRelacion(relacionAgregarSolucion)
                    estadosSolucion.append(relacionAgregarSolucion)
                    solucionEncontrada = relacion
                    fin = True
                    break
                elif relacion.valor < estadoActual.valor and relacion.valor < sucesor:
                    estadoNuevo = relacion
    ...
    def calcularDistanciaEuclidea(self, a, b):
        ...
        return round(distanciaEuclidea, 3)
    ...
    def calcularDistanciaManhattan(self, a, b):
        ...
        return round(distanciaManhattan, 3) 
